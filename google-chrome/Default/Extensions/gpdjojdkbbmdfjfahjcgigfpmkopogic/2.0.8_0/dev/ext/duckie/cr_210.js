  ...
  var $ = w[a.k] = {
    ...
    'f': (function () {
      return {
                ...
                if (el.src) {
                  if (!$.f.get(el, 'nopin') && !$.f.get(el, 'data-pin-nopin')) {
                    if (!$.f.get(el, 'data-pin-no-hover')) {
                      ...
                      // watch for larger images resized downwards by browser
        ...
        // turn context menus on or off depending on the page we're on
        updateSearchSetting: function () {
        ...
        canHazSearch: function () {
          ...
          // check for no-search meta
          $.f.metaCheck();

          // hideSearch overrides alwaysCanHazSearch debug setting
          if (!$.v.hideSearch) {
            if ($.v.hazLogin) {
              // if we find a Pinterest app domain, which has three or less numbers or letters, an optional dot, and then pinterest.com
              if ($.d.URL.match($.a.urlPattern.pinterestApp)) {
          ...
          // either way, always update search setting to turn on / off context menu
          $.f.updateSearchSetting();
